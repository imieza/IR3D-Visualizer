...
class MainView(QtGui.QMainWindow):
    ...
    def calculate(self):
        if self.fileName[0] != []:
            ...
            #self.calc['intensity_windows'][self.calc['peaks'][0]]
            self.dataProcceser.refactoring_time()
            self.ui.progressBar.setValue(45)
            print "i_db len: ", len(self.calc["i_db"])
            print "intensity_windows len: ", len(self.calc["intensity_windows"][0,:])
            print "data len: ", len(self.calc["data"].tolist()[0])
            self.calc["normalizado"] = self.dataProcceser.normalizer(self.calc["i_db"][self.calc["peaks"]])
